...
const String feedbackUrl = 'https://mirc.app/feedback';
...
class _mircHomePageState extends State<mircHomePage> with WidgetsBindingObserver {
  ...
  late StreamSubscription<List<ConnectivityResult>> _connectivitySub; // Updated type
  ...
  void initState() {
    ...
    /// Listen for connectivity changes (Offline banner)
    _connectivitySub = Connectivity().onConnectivityChanged.listen((List<ConnectivityResult> results) {
      // Check if we have any valid internet connection
    ...
    log.info('Internet restored, reloading podcasts');
    _initialSubscriptionsLoaded = false;
    _loadMultiplePodcastsInOrder();
  }
    });
  }
  ...
  void _loadMultiplePodcastsInOrder() async {
    ...
    if (_isOffline) {
      log.info('Offline mode: skipping podcast loading');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        if (successfulSubscriptions > 0 || failedSubscriptions > 0 && !_isOffline) {
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        body: Column(
          children: <Widget>[
            // Offline banner positioned right below the app bar
            if (_isOffline)
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const Expanded(
                      child: Text(
                        'Offline Mode - Showing downloaded content only',
                        style: TextStyle(
                          color: Colors.white,
